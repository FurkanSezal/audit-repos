...
contract RankedChoice is EIP712 {
    ...
    // @audit-medium this should be "rankCandidates(address[])" this is not a struct
    bytes32 public constant TYPEHASH = keccak256("OrderedCandidates(rankCandidates(address[]))");
    uint256 private constant MAX_CANDIDATES = 10;

    // Solidity doesn't support contant reference types
    address[] private VOTERS;
    mapping(address voter => mapping(uint256 voteNumber => address[] orderedCandidates)) private s_rankings;

    // For selecting the president
    address[] private s_candidateList;
    mapping(address candidate => mapping(uint256 voteNumber => mapping(uint256 roundId => uint256 votes))) private
        s_candidateVotesByRound;
    ...
    function selectPresident() external {
        ...
        // @audit-info this should be cache VOTERS.length
        for (uint256 i = 0; i < VOTERS.length; i++) {
            address[] memory orderedCandidates = s_rankings[VOTERS[i]][s_voteNumber];
            //@audit-info this should be cache orderedCandidates.length
            // 0x00 0x01 0x02
        ...
        returns (address[] memory)
    {
        ...
        address[] memory newCandidateList = new address[](candidateList.length - 1);
    ...
    /*//////////////////////////////////////////////////////////////
                                GETTERS
    //////////////////////////////////////////////////////////////*/
    function getUserCurrentVote(address voter) external view returns (address[] memory) {
